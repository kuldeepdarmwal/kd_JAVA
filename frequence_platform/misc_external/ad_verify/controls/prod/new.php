...
<body>
<h2>Screenshot Bot v.2</h2>
<p>New Version: Picks sites from top placements and matches to pages on those sites related to selected topics</p>
   <table>
   <tr>
   <td>Campaign: </td>
...
/*
	$top_site_query = "SELECT d.url AS Site  FROM ad_verify_page_data d JOIN ad_verify_page_categories c ON (d.id = c.page_id) WHERE c.category_id IN (SELECT tag_id FROM ad_verify_campaign_categories WHERE campaign_id = ".$a_campaign['id'].") AND d.base_url IN (SELECT b.Base_site AS Site                                                                       
FROM                                                                                                                  
AdGroups a LEFT JOIN SiteRecords b ON (a.ID = b.AdGroupID)                                                            
                                                                                                                      
WHERE                                                                                                                 
b.Base_site != 'All other sites' AND                                                                                                                                                   
a.campaign_id = ".$a_campaign['id']." AND                                                                    
b.Date > '2013-01-01'                                                                                                 
GROUP BY Site ORDER BY SUM(b.Impressions) DESC) LIMIT 30";

*/
	$top_site_query = "SELECT d.url AS Site  FROM ad_verify_page_data d JOIN ad_verify_page_categories c ON (d.id = c.page_id) WHERE c.category_id IN (SELECT tag_id FROM ad_verify_campaign_categories WHERE campaign_id = ".$a_campaign['id'].") LIMIT 20";


	if($a_campaign['id'] == '253')
	{
	    echo $top_site_query;
	}
...
<table>
   <tr>
   <td>Max # of Checks</td>
   <td><input type="text" size="5" id="max_loops" value="20"></td>
