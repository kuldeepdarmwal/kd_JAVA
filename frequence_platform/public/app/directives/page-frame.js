System.register(['angular2/core', '../services/config'], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var __param = (this && this.__param) || function (paramIndex, decorator) {
        return function (target, key) { decorator(target, key, paramIndex); }
    };
    var core_1, config_1;
    var PageFrame;
    return {
        setters:[
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (config_1_1) {
                config_1 = config_1_1;
            }],
        execute: function() {
            PageFrame = (function () {
                function PageFrame(el, config) {
                    this.el = el;
                    this.config = config;
                    this.onPageLoad = new core_1.EventEmitter();
                    this._el = el.nativeElement;
                    this.onPageLoad.emit(this);
                }
                PageFrame.prototype.onLoad = function () {
                    this.onPageLoad.emit(this);
                    this.onResize();
                };
                PageFrame.prototype.onResize = function () {
                    var container_width = this._el.offsetWidth;
                    var page = this._el.contentWindow.document.querySelector('.page');
                    if (page) {
                        this.pageWidth = page.offsetWidth;
                        page.setAttribute('style', "margin:0;");
                    }
                    else {
                        page = { offsetWidth: this.config.page_width, offsetHeight: this.config.page_height };
                    }
                    var ratio = container_width / page.offsetWidth;
                    if (ratio >= 1) {
                        this._el.contentWindow.document.body.setAttribute('style', "transform: none; width: 100%;");
                        return true;
                    }
                    if (this._el.contentWindow.document.body !== null) {
                        this._el.contentWindow.document.body.setAttribute('style', "\n\t\t\t\twidth: " + page.offsetWidth + "px;\n\t\t\t\ttransform: scale(" + ratio + ");\n\t\t\t\ttransform-origin: 0px 0px 0px;\n\t\t\t\toverflow: hidden");
                    }
                    this._el.setAttribute('style', "height: " + page.offsetHeight * ratio + "px");
                };
                __decorate([
                    core_1.Output(), 
                    __metadata('design:type', Object)
                ], PageFrame.prototype, "onPageLoad", void 0);
                PageFrame = __decorate([
                    core_1.Directive({
                        selector: 'iframe[pageframe]',
                        host: {
                            '(load)': 'onLoad()',
                            '(window:resize)': 'onResize($event)'
                        }
                    }),
                    __param(1, core_1.Inject(config_1.ConfigService)), 
                    __metadata('design:paramtypes', [core_1.ElementRef, Object])
                ], PageFrame);
                return PageFrame;
            }());
            exports_1("PageFrame", PageFrame);
        }
    }
});

//# sourceMappingURL=page-frame.js.map
