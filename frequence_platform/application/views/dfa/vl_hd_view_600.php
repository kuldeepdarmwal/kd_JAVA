...
function vl() {
	...
	if(isIE) {
		...
		if(false && ieVersion < 10) { // TODO: evaluate what versions of IE we'll support and what to do if we encounter an older version
	...
	for( var num=0; num < opt.numSlides; num++ ) {
		...
		if( num == 0 && opt.isExpandable == true && num == opt.expandSlide-1 ) {
			...
			if ( num == opt.expandSlide-1 && opt.isExpandable == true ) {
				...
				slides += '<img id="'+ad.clientID+'_slide_image_'+num+'" class="vl-slide-image" src="'+ ad.fileContainer + slideImages[num] +'" onclick="'+ad.clientID+'_adbox.'+'clickHandler(0); '+ad.clientID+'_adbox.'+'onEnrollButtonClick();"/>'
				...
				if(opt.useSpecLogo == 'true'){
				    ...
				    slides += '<div id="'+ad.clientID+'_specimage_'+num+'" onclick="'+ad.clientID+'_adbox.'+'onEnrollButtonClick();" style="background-color:#AAAAAA;width:'+ad.width+'px;position:absolute;top:5px;height:'+opt.speclogoheight+'px;text-align:'+opt.speclogoalign+';">';
				...
				slides += '<div id="'+ad.clientID+'_nav_'+num+'" style="background-color:#CCCCCC;width:'+ad.width+'px;height:28px;position:absolute;top:'+(ad.height-28)+'px;">'
			    ...
			    <img id="'+ad.clientID+'_nav_'+num+'_0" class="vl-slidenavbtn" src="/images/navbar/video.png" onclick="'+ad.clientID+'_adbox.'+'slider.slide(0, 300)">\n\
			    <img id="'+ad.clientID+'_nav_'+num+'_1" class="vl-slidenavbtn" src="/images/navbar/map.png" onclick="'+ad.clientID+'_adbox.'+'slider.slide(1, 300)">\n\
			    <img id="'+ad.clientID+'_nav_'+num+'_2" class="vl-slidenavbtn" src="/images/navbar/social.png" onclick="'+ad.clientID+'_adbox.'+'slider.slide(2, 300)">\n\
			    <img id="'+ad.clientID+'_nav_'+(num+1)+'_CTA" class="vl-ctaslidenavbtn" src="/images/navbar/cta.png" onclick="'+ad.clientID+'_adbox.'+'onEnrollButtonClick();"></div>';
	...
	//loadScript ( 'https://ed18a41e851d21bd3d3b-2105a7941d639194e8ba5253efec10c9.ssl.cf1.rackcdn.com/js/swipejs/swipe_mod_5.3.js', 'js', "swipejs" );
