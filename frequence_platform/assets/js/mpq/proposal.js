;(function($){
	...
	function encode_selected_industry_id()
	{
		var raw_industry_data = $('#industry_select').select2('val');
		return JSON.stringify(raw_industry_data);
	}
	...
	*/
	...
	function handle_submit_mpq(is_valid, validation_messages)
	{
		...
		if(is_valid.is_valid === true)
		{
			...
			var industry_id=encode_selected_industry_id();
			...
			$.ajax({
				...
				{   
				    ...
				    else
				    {
						...
						{
							$('#submit_modal').hide();
							$("body").append('<form action="/embed/proposal/summary" name="mpq_is_submitted" method="post" style="display:none;"><input id="mpq_is_submitted_json_input" type="text" name="submitted_data" value="" /><input id="cc_email" type="text" name="cc_email" value="" /><input id="insertion_order_id" name="insertion_order_id"/></form>');
							$("#mpq_is_submitted_json_input").val(JSON.stringify(data["submit_data"]));
							$('input[name="proposal_id"').val(data['proposal_id']);
