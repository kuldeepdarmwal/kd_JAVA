    ...
    $.fn.extend({
        strackbar: function(options) {
            ...
            return this.each(function() {
                ...
                var slider = $('<div></div>').attr('id', 'slider').addClass('slider_class');
                var selection = $('<div></div>').attr('id', 'inner').html('&nbsp').addClass('inner_slider');
                ...
                ltracker.mouseup(function(e) {
										if(mousecaptured == true && !settings.triggerCallbackOnMove) {
                    	mousecaptured = false;
                    	if (settings.callback != null)
											{
                ...
                function setTrackerPosition(cursorPosition) {
                    ...
                    if (cursorPosition >= sliderLeft && cursorPosition <= (sliderRight - stepValue / 10)) {
                        if (trackerPosition >= 0 && trackerPosition <= settings.sliderWidth - stepValue / 10) {
                            ...
                            if (settings.callback != null) {
																if(mousecaptured == false || settings.triggerCallbackOnMove) {
